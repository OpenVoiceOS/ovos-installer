...
if [ -z "${OVERCLOCK_CURRENT_VALUES_TITLE:-}" ]; then
  OVERCLOCK_CURRENT_VALUES_TITLE="Current overclock values:"
fi

if [ -z "${OVERCLOCK_CURRENT_ARM_FREQ_LABEL:-}" ]; then
  OVERCLOCK_CURRENT_ARM_FREQ_LABEL="arm_freq"
fi

if [ -z "${OVERCLOCK_CURRENT_GPU_FREQ_LABEL:-}" ]; then
  OVERCLOCK_CURRENT_GPU_FREQ_LABEL="gpu_freq"
fi

if [ -z "${OVERCLOCK_CURRENT_OVER_VOLTAGE_LABEL:-}" ]; then
  OVERCLOCK_CURRENT_OVER_VOLTAGE_LABEL="over_voltage"
fi

if [ -z "${OVERCLOCK_CURRENT_INITIAL_TURBO_LABEL:-}" ]; then
  OVERCLOCK_CURRENT_INITIAL_TURBO_LABEL="initial_turbo"
fi

if [ -z "${OVERCLOCK_CURRENT_ARM_BOOST_LABEL:-}" ]; then
...
${OVERCLOCK_CURRENT_VALUES_TITLE}
  - ${OVERCLOCK_CURRENT_ARM_FREQ_LABEL}: ${OVERCLOCK_ARM_FREQ}
  - ${OVERCLOCK_CURRENT_GPU_FREQ_LABEL}: ${OVERCLOCK_GPU_FREQ}
  - ${OVERCLOCK_CURRENT_OVER_VOLTAGE_LABEL}: ${OVERCLOCK_OVER_VOLTAGE}
  - ${OVERCLOCK_CURRENT_INITIAL_TURBO_LABEL}: ${OVERCLOCK_INITIAL_TURBO}
  - ${OVERCLOCK_CURRENT_ARM_BOOST_LABEL}: ${OVERCLOCK_ARM_BOOST}"
